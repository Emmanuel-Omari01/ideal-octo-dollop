 ...
 *                                                                           *
 *  (Copyright) 2007 - 2008 Sven Stefan Krauss                               *
...
#ifndef SRC_OPENSOURCE_UCUNIT_UCUNIT_V1_0_H_
#define SRC_OPENSOURCE_UCUNIT_UCUNIT_V1_0_H_
#include <stdbool.h>

void System_Safestate();
 ...
 */
#define UCUNIT_WriteString(msg)    termFilterPtr_->print(msg)     // previous call "term_ptr->printFormat"
                                                                  // with replacement of "\n"
#define UCUNIT_WriteLine(msg)      termFilterPtr_->println(msg)

/**
 * @Macro:       UCUNIT_Write(chr)
 *
 * @Description: Encapsulates a function which is called for
 *               writing a single character to the host computer.
 *
 * @param chr:   Character which shall be written.
 *
 *               For most microcontrollers a special implementation of
 *               printf is available for writing to a serial
 *               device or network. In some cases you will have
 *               also to implement a putch(char c) function.
 */
#define UCUNIT_Write(chr)     termFilterPtr_->putchar(chr)
...
#define UCUNIT_WriteInt(n)    termFilterPtr_->printIntln(n)
 ...
 * UCUNIT_MODE_NORMAL: Only checks that fail are displays
...
#define UCUNIT_ACTION_WARNING   0 /* Goes through the checks with message depending on level */
#define UCUNIT_ACTION_SHUTDOWN  1 /* Stops on the end of the checklist if any check has failed */
...
// AME defined variables
static bool ucunit_last_test = true;
...
#define UCUNIT_TestcaseEnd()                                         \
    ...
    {                                                                \
        ...
        if (0 == (ucunit_testcases_failed_checks - ucunit_checks_failed)) { \
            UCUNIT_WriteString("Testcase passed.\r");                \
...
#define UCUNIT_Tracepoint(index)                         \
    if (index < UCUNIT_MAX_TRACEPOINTS) {                \
        ...
        UCUNIT_WriteFailedMsg("Trace point index", #index);  \
    }
...
{                                                                     \
    UCUNIT_WriteString("\r**************************************");   \
    UCUNIT_WriteString("\rTestcases: failed: ");                      \
    UCUNIT_WriteInt(ucunit_testcases_failed);                         \
    UCUNIT_WriteString("\r           passed: ");                      \
    UCUNIT_WriteInt(ucunit_testcases_passed);                         \
    UCUNIT_WriteString("\rChecks:    failed: ");                      \
...
// AME function prototype(s) declaration
void UCUnit_setLastTest(bool set);  // define to suppress warning
bool UCUnit_getLastTest(void);      // determine if last test is running

#endif  // SRC_OPENSOURCE_UCUNIT_UCUNIT_V1_0_H_
