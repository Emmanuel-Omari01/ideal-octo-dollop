...
static void bsp_lvd1_resetting(void);
...
{
    /* Execute countermeasures for RENESAS TECHNICAL Document No. UPDATE TN-RX*-A153A/E. */
    if(0 == (BSP_CFG_OFS1_REG_VALUE & 0x00000002))
    {
        /* When the voltage monitoring 1 reset by the option-setting memory is automatically started with 
           the use of the IWDT reset. */
        if((0x00001000 == (BSP_CFG_OFS0_REG_VALUE & 0x00001002)) || (1 == IWDT.IWDTRCR.BIT.RSTIRQS))
        {
            bsp_lvd1_resetting();
        }
    }
...
static void clock_source_select (void)
{
    ...
    if (0 == SYSTEM.RSTSR1.BIT.CWSF)
    {
        ...
        /* Set the drive capacity of the sub-clock oscillator */
            ...
            tmp = 0x02;
        #elif (BSP_CFG_SOSC_DRV_CAP == 2) /* Medium CL */
            tmp = 0x00;
        #elif (BSP_CFG_SOSC_DRV_CAP == 3) /* High CL */
            tmp = 0x01;
        #else
...
/***********************************************************************************************************************
* Function name: bsp_lvd1_resetting
* Description  : This function is a countermeasure function for "TN-RX*-A153A/E".
*              : Note:
*              :   The voltage monitoring 1 reset may not be generated after a reset by the independent
*              :   watchdog timer (IWDT) when the voltage monitoring 1 reset is enabled after a reset 
*              :   release by setting the OFS1.STUPLVD1REN bit in the option-setting memory to 0. 
*              : Countermeasure:
*              :   When the voltage monitoring 1 reset by the option-setting memory is automatically
*              :   started with the use of the IWDT reset, enable the voltage monitoring 1 reset by 
*              :   a program immediately after a reset.
* Arguments    : none
* Return value : none
***********************************************************************************************************************/
static void bsp_lvd1_resetting(void)
{
    ...
    SYSTEM.LVDLVLR.BIT.LVD1LVL = ((BSP_CFG_OFS1_REG_VALUE & 0x000000F0) >> 4);

    /* Set LVD1RN and LVD1R */
    dummy = SYSTEM.LVD1CR0.BYTE;
    dummy &= 0x45;                 /* LVD1RN = 0, b3 = 0 */
    dummy |= 0x40;                 /* LVD1RI = 1 */
    SYSTEM.LVD1CR0.BYTE = dummy;
    ...
    dummy = SYSTEM.LVD1CR0.BYTE;
    dummy &= 0xC5;                /* b3 = 0 */
    dummy |= 0x04;                /* LVD1CMPE = 1 */
    SYSTEM.LVD1CR0.BYTE = dummy;

    /* Enable protect bit */
    SYSTEM.PRCR.WORD = 0xA500;
} /* End of function bsp_lvd1_resetting() */
