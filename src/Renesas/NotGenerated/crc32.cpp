 ...
 *	@File: crc32.cpp
 *	@Brief:
 *	this file is used to define the crc32 function used when communicating with the cpx3
 *
 */
...
namespace CRC32 {
const uint32_t crc32_table[256] = { //!< Used for the CRC32 computation
 ...
 *	\brief initialises crc32 lookup table (crc32_table[256])
 *	\return success
 */
uint32_t crc32_init() {
    /***
...
uint32_t crc32_calc(const uint8_t *frame, uint16_t frame_length) {
    ...
    for (byte = 0u; byte < frame_length; ++byte) {
        data = (uint8_t) ((*(frame + byte)) ^ (residue >> (CRC_WIDTH - 8u)));
 ...
 */
bool crc32_check(const uint8_t *frame, uint16_t frame_length,
        uint32_t crc32_to_check) {
    bool crc_check = false;

    if (crc32_to_check == crc32_calc(frame, frame_length)) {
        crc_check = true;
    } else {
        crc_check = false;
    }
